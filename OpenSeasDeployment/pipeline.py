...
def create_new_node(slug):
    ...
    videoHash = subprocess.check_output('python3 scripts/advanced_collectible/create_nft_from_twitch.py '+path_to_downloaded_video, shell=True, universal_newlines=True)
    videoHash=videoHash.split()[-1]
    print("Ipfs Hash of the Video is: "+videoHash)
    write_file_for_metadata('nonfungbotStreamer','dev work stream',videoHash)
    time.sleep(3)
    os.system('brownie run scripts/advanced_collectible/create_collectible.py --network rinkeby')
    time.sleep(3)
    output = subprocess.check_output("brownie run scripts/advanced_collectible/create_metadata.py --network rinkeby", shell=True, universal_newlines=True)
    ...
    f.close()   
    time.sleep(3)
    outputOfUploading = subprocess.check_output("brownie run scripts/advanced_collectible/set_tokenuri.py --network rinkeby", shell=True, universal_newlines=True)
